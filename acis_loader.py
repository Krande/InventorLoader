...
def NORM(value):
    return VEC(value.x, value.y, value.z).normalize()
...
def DCUR(c, x):
    return c.parameter(x)


def DSURF(c, u, v):
    return c.parameter(u, v)
...
class Law(object):
    ...
    def evaluate(self, X):
        try:
            ...
            logError("    Can't evaluate '%s': %s", self.eq, e)
        return None
    ...
    try:
        ...
        while i < len(types):
            try:
                ...
                i += 1
        logError("    Missing class implementation for '%s' - using base class '%s'!", entity.name, t)
...
def readPoints2DList(nubs, count, chunks, index):
    ...
    for k in range(0, us):
        ...
        if nubs.rational:
            nubs.weights[k], i = getFloat(chunks, i)
...
def isOnLine(sEdge, fEdge):
    ...
    if isEqual(sp[0], fp[0]):
        return isBetween(sp[0], fp[1], sp[1])
    if isEqual(sp[1], fp[1]):
        return isBetween(sp[0], fp[0], sp[1])
    return False
...
class BS3_Curve(object):
    ...
    def readPoints3DList(self, count, chunks, index):
        ...
        for u in range(0, us):
            ...
            if self.rational:
                self.weights[u], i = getFloat(chunks, i)

        self.uKnots, self.uMults, self.uPeriodic = adjustMultsKnots(
            self.uKnots, self.uMults, self.uPeriodic, self.uDegree
        )
        return i
    ...
    def readPoints3DMap(self, countU, countV, chunks, index):
        ...
        for v in range(0, vs):
            for u in range(0, us):
                ...
                if self.rational:
                    self.weights[u][v], i = getFloat(chunks, i)

        self.uKnots, self.uMults, self.uPeriodic = adjustMultsKnots(
            self.uKnots, self.uMults, self.uPeriodic, self.uDegree
        )
        self.vKnots, self.vMults, self.vPeriodic = adjustMultsKnots(
            self.vKnots, self.vMults, self.vPeriodic, self.vDegree
        )
...
class Range(object):
    ...
    def __str__(self):
        return "I" if (self.type == "I") else "F %g" % (self.getLimit())

    def __repr__(self):
        return "I" if (self.type == "I") else "%g" % (self.getLimit())

    def getLimit(self):
        return self.limit if (self.type == "I") else self.limit * self.scale

    def __eq__(self, other):
        if isinstance(other, Range):
            return self.getLimit() == other.getLimit()
...
class Interval(object):
    ...
    def getLowerLimit(self):
        return self.lower.getLimit()

    def getUpperType(self):
        return self.upper.type

    def getUpperLimit(self):
        return self.upper.getLimit()

    def getLimit(self):
        return self.getUpperLimit() - self.getLowerLimit()
...
class Bulletin(object):
    ...
    def __str__(self):
        if self.old.val == -1:
            return "$-1   $%-4d // inserted:   %s" % (self.new.val, self.new.entity)
        if self.new.val == -1:
            return "$%-4d $-1   // deleted:    %s" % (self.old.val, self.old.entity)
        return "$%-4d $%-4d // updated id: %s" % (self.old.val, self.new.val, self.new.entity)
...
class BulletinBoard(object):
    ...
    def __str__(self):
        return "BulletinBoard: %d" % (self.number)
...
class DeltaState(object):
    ...
    def getPrevious(self):
        return self.previous.entity

    def getNext(self):
        return self.next.entity

    def getPartner(self):
        return self.partner.entity

    def getMerged(self):
        return self.merged.entity
    ...
    def __str__(self):
        return "delta_state %d %d %d %s %s %s %s %s %s" % (
            self.id,
            self.rollbacks,
            self.hidden,
            self.previous,
    ...
    def __repr__(self):
        return "%d delta_state %d %d %d %s %s %s %s %s %s" % (
            self.index,
            self.id,
            self.rollbacks,
            self.hidden,
...
class EndOfAcisHistorySection(object):
    pass


class EndOfAcisData(object):
    pass
...
class Entity(object):
    ...
    @property
    def index(self):
        return -1 if (self.entity is None) else self.entity.index

    def getType(self):
        return -1 if (self.entity is None) else self.entity.name

    @property
    def attrib(self):
        return None if (self._attrib is None) else self._attrib.node

    def __str__(self):
        return "%s" % (self.entity)

    def __repr__(self):
        return "%r" % (self.entity)
    ...
    def getSatTextSub(self, index):
        return "".join("%s" % (c) for c in self.entity.chunks[index:])
...
class EyeRefinement(Entity):
    def __init__(self):
        super(EyeRefinement, self).__init__()


class VertexTemplate(Entity):
    def __init__(self):
        super(VertexTemplate, self).__init__()


class Wcs(Entity):
    def __init__(self):
        super(Wcs, self).__init__()
...
class Body(Topology):
    ...
    def getLump(self):
        return None if (self._lump is None) else self._lump.node

    def getWire(self):
        return None if (self._wire is None) else self._wire.node

    def getTransform(self):
        return None if (self._transform is None) else self._transform.node
...
class Lump(Topology):
    ...
    def getNext(self):
        return None if (self._next is None) else self._next.node

    def getShell(self):
        return None if (self._shell is None) else self._shell.node

    def getOwner(self):
        return None if (self._owner is None) else self._owner.node
...
class Shell(Topology):
    ...
    def getFace(self):
        return None if (self._face is None) else self._face.node
...
class SubShell(Topology):
    ...
    def getChild(self):
        return None if (self._child is None) else self._child.node
...
class Face(Topology):
    ...
    def getLoop(self):
        return None if (self._loop is None) else self._loop.node

    def getParent(self):
        return None if (self._parent is None) else self._parent.node

    def getSurface(self):
        return None if (self._surface is None) else self._surface.node
    ...
    def isCone(self):
        return isinstance(self.getSurface(), SurfaceCone)

    def isMesh(self):
        return isinstance(self.getSurface(), SurfaceMesh)

    def isPlane(self):
        return isinstance(self.getSurface(), SurfacePlane)

    def isSphere(self):
        return isinstance(self.getSurface(), SurfaceSphere)

    def isSpline(self):
        return isinstance(self.getSurface(), SurfaceSpline)

    def isTorus(self):
        return isinstance(self.getSurface(), SurfaceTorus)
...
class Loop(Topology):
    ...
    def getCoEdge(self):
        return None if (self._coedge is None) else self._coedge.node
...
class CoEdge(Topology):
    ...
    def getPrevious(self):
        return None if (self._previous is None) else self._previous.node

    def getPartner(self):
        return None if (self._partner is None) else self._partner.node

    def getEdge(self):
        return None if (self._edge is None) else self._edge.node

    def getOwner(self):
        return None if (self._owner is None) else self._owner.node

    def getCurve(self):
        return None if (self._curve is None) else self._curve.node
...
class Edge(Topology):
    ...
    def getStart(self):
        return None if (self._start is None) else self._start.node.getPosition()

    def getEnd(self):
        return None if (self._end is None) else self._end.node.getPosition()
    ...
    def getPoints(self):
        ...
        if ptStart is not None:
            points.append(ptStart.getPosition())
        ptEnd = None if (self._end is None) else self._end.node
        if (ptEnd is not None) and (ptEnd.index != ptStart.index):
            points.append(ptEnd.getPosition())
        return points
...
class Vertex(Topology):
    ...
    def getPoint(self):
        return None if (self._point is None) else self._point.node
...
class CurveEllipse(Curve):  # ellyptical curve "ellipse-curve"
    ...
    def __repr__(self):
        return "Curve-Ellipse: center=%s, dir=%s, major=%s, ratio=%g, range=%s" % (
            self.center,
            self.axis,
            self.major,
            self.ratio,
...
class CurveInt(Curve):  # interpolated ('Bezier') curve "intcurve-curve"
    ...
    def __str__(self):
        return "%s %s {%s ...}" % (self.__name__, SENSE.get(self.sense, self.sense), self.subtype)
    ...
    def setCurve(self, chunks, index):
        ...
        if self.singularity == "full":
            ...
            if (getVersion() >= 16.0) and (isASM() == False):
                i += 1  # 3
            arr, i = getFloatArray(chunks, i)
    ...
    def setOff(self, chunks, index, inventor):
        ...
        if inventor:
            i += 1
        if (getVersion() > 22.0) and (isASM() == False):
            i += 3  # -1 none F
    ...
    def setSubtype(self, chunks, index):
        ...
        assert chunks[i].tag == TAG_SUBTYPE_CLOSE, "-%s %s - pending chunks to read: %s" % (
            self.index,
            self.subtype,
            chunks[i:],
        )
    ...
    def build(self, start=None, end=None):
        if self.__ready_to_build__:
            ...
            if self.shape is None:
                if self.subtype == "ref":
                    logWarning(
                        "    ... Don't know how to build '-%d %s { ref %d }'!", self.index, self.__name__, self.ref
                    )
                else:
                    logWarning(
                        "    ... Don't know how to build '-%d %s { %s ... }'!", self.index, self.__name__, self.subtype
                    )
        return self.shape
...
class CurveIntInt(CurveInt):  # interpolated int-curve "intcurve-intcurve-curve"
    ...
    def setBulk(self, chunks, index):
        ...
        if (getVersion() >= 25.0) and (isASM() == False):
            id, i = getInteger(chunks, i)  # subtype table index
        getReader().addSubtypeNode(self)
        if self.subtype == "lawintcur":
            return self.setLaw(chunks, i, False)
        if self.subtype == "law_int_cur":
            return self.setLaw(chunks, i + 1, True)
...
class CurveStraight(Curve):  # straight curve "straight-curve"
    ...
    def getSatTextGeometry(self, index):
        return "%s %s %s #" % (vec2sat(self.root), vec2sat(self.dir), self.range)

    def __repr__(self):
        return "Line: root=%s, dir=%s, range=%s" % (self.root, self.dir, self.range)
...
class SurfaceCone(Surface):
    ...
    def __repr__(self):
        return "Surface-Cone: center=%s, axis=%s, radius=%g, ratio=%g, semiAngle=%g" % (
            self.center,
            self.axis,
            self.major.Length,
            self.ratio,
            degrees(asin(self.sine)),
        )
    ...
    def build(self, face=None):
        if self.__ready_to_build__:
            self.__ready_to_build__ = False
            if isEqual1D(self.sine, 0.0):  # 90 Deg
                if isEqual1D(self.ratio, 1.0):
                ...
                if not isEqual1D(self.ratio, 1.0):
...
class SurfacePlane(Surface):
    ...
    def __repr__(self):
        return "Surface-Plane: root=%s, normal=%s, uvorigin=%s" % (self.root, self.normal, self.uvorigin)
...
class SurfaceSphere(Surface):
    ...
    def __repr__(self):
        return "Surface-Sphere: center=%s, radius=%g, uvorigin=%s, pole=%s" % (
            self.center,
            self.radius,
            self.uvorigin,
            self.pole,
...
class SurfaceSpline(Surface):
    ...
    def setCylinder(self, chunks, index, inventor):
        ...
        if profile:
            self.major = profile.valueAt(0) - self.center
        ...
        if self.__ready_to_build__:
            ...
            if self.shape is None:
                if hasattr(self, "subtype"):
                    if self.subtype != "ref":
                        logWarning(
                            "    ... Don't know how to build surface '-%d %s { %s ... }' - only edges displayed!",
                            self.index,
                            self.type,
                            self.subtype,
                        )
                        ...
                        logWarning(
                            "    ... Don't know how to build surface '-%d %s { ref %d }' - only edges displayed!",
                            self.index,
                            self.type,
                            self.ref,
                        )
                    ...
                    logWarning(
                        "    ... Don't know how to build surface '-%d %s' - only edges displayed!",
                        self.index,
                        self.type,
                    )
...
class SurfaceTorus(Surface):
    ...
    def __repr__(self):
        return "Surface-Torus: center=%s, normal=%s, R=%g, r=%g, uvorigin=%s" % (
            self.center,
            self.axis,
            self.major,
            self.minor,
    ...
    def build(self, face=None):
        if self.__ready_to_build__:
            ...
            try:
                ...
                logWarning(
                    "Can't create torus for center=%s, axis=%s, major=%g, minor=%g, UV=%s: %s"
                    % (self.center, self.axis, self.major, self.minor, self.uvorigin, e)
                )
        return self.shape
...
class Point(Geometry):
    ...
    def getSatTextGeometry(self, index):
        return "%s #" % (vec2sat(self.position))
...
class RhEntityRhMaterial(RhEntity):
    ...
    # @5 color -4 0 0 0 #
    def __init__(self):
        super(RhEntityRhMaterial, self).__init__()
...
class AttribBt(Attrib):
    def __init__(self):
        super(AttribBt, self).__init__()


class AttribBtEntityColor(AttribBt):
    # string with numbers
    def __init__(self):
        super(AttribBtEntityColor, self).__init__()


class AttribADesk(Attrib):
    def __init__(self):
        super(AttribADesk, self).__init__()
...
class AttribAnsoft(Attrib):
    def __init__(self):
        super(AttribAnsoft, self).__init__()


class AttribAnsoftId(AttribAnsoft):
    def __init__(self):
        super(AttribAnsoftId, self).__init__()


class AttribAnsoftProperties(AttribAnsoft):
    def __init__(self):
        super(AttribAnsoftProperties, self).__init__()


class AttribDxid(Attrib):
    def __init__(self):
        super(AttribDxid, self).__init__()


class AttribCustom(Attrib):
    def __init__(self):
        super(AttribCustom, self).__init__()


class AttribDesigner(Attrib):
    def __init__(self):
        super(AttribDesigner, self).__init__()


class AttribDesignerHistory(AttribDesigner):
    # 0, 0
    def __init__(self):
        super(AttribDesignerHistory, self).__init__()


class AttribDesignerSurfaceId(AttribDesigner):
    # 0, 65539, 0
    def __init__(self):
        super(AttribDesignerSurfaceId, self).__init__()


class AttribDesignerOwnerTag(AttribDesigner):
    # no more values!
    def __init__(self):
        super(AttribDesignerOwnerTag, self).__init__()


class AttribEye(Attrib):
    def __init__(self):
        super(AttribEye, self).__init__()


class AttribEyeFMesh(AttribEye):
    def __init__(self):
        super(AttribEyeFMesh, self).__init__()


class AttribEyePtList(AttribEye):
    def __init__(self):
        super(AttribEyePtList, self).__init__()


class AttribEyeRefVt(AttribEye):
    def __init__(self):
        super(AttribEyeRefVt, self).__init__()


class AttribFdi(Attrib):
    def __init__(self):
        super(AttribFdi, self).__init__()


class AttribFdiLabel(AttribFdi):
    def __init__(self):
        super(AttribFdiLabel, self).__init__()


class AttribGen(Attrib):
    def __init__(self):
        super(AttribGen, self).__init__()
...
class AttribKcId(Attrib):
    # string with numbers
    def __init__(self):
        super(AttribKcId, self).__init__()


class AttribLwd(Attrib):
    def __init__(self):
        super(AttribLwd, self).__init__()


class AttribLwdFMesh(Attrib):  # color face mapping
    def __init__(self):
        super(AttribLwdFMesh, self).__init__()


class AttribLwdPtList(Attrib):  # Refinement-VertexTemplate mapping for body
    def __init__(self):
        super(AttribLwdPtList, self).__init__()


class AttribLwdRefVT(Attrib):  # Refinement-VertexTemplate mapping for body
    def __init__(self):
        super(AttribLwdRefVT, self).__init__()


class AttribRBase(Attrib):
    def __init__(self):
        super(AttribRBase, self).__init__()


class AttribRBaseRender(AttribRBase):
    def __init__(self):
        super(AttribRBaseRender, self).__init__()


class AttribRfBase(Attrib):
    def __init__(self):
        super(AttribRfBase, self).__init__()


class AttribRfBaseFaceTracker(AttribRfBase):
    def __init__(self):
        super(AttribRfBaseFaceTracker, self).__init__()


class AttribSg(Attrib):
    def __init__(self):
        super(AttribSg, self).__init__()


class AttribSgPidName(AttribSg):
    def __init__(self):
        super(AttribSgPidName, self).__init__()


class AttribSnl(Attrib):
    def __init__(self):
        super(AttribSnl, self).__init__()


class AttribSnlCubitOwner(AttribSnl):
    def __init__(self):
        super(AttribSnlCubitOwner, self).__init__()


class AttribSt(Attrib):
    def __init__(self):
        super(AttribSt, self).__init__()


class AttribStNoMerge(AttribSt):
    def __init__(self):
        super(AttribStNoMerge, self).__init__()


class AttribStNoCombine(AttribSt):
    def __init__(self):
        super(AttribStNoCombine, self).__init__()
...
class AttribStDisplay(AttribSt):
    def __init__(self):
        super(AttribStDisplay, self).__init__()


class AttribStId(AttribSt):
    def __init__(self):
        super(AttribStId, self).__init__()


class AttribSys(Attrib):
    def __init__(self):
        super(AttribSys, self).__init__()


class AttribSysAnnotationAttrib(AttribSys):
    def __init__(self):
        super(AttribSysAnnotationAttrib, self).__init__()


class AttribSysConvexity(AttribSt):
    def __init__(self):
        super(AttribSysConvexity, self).__init__()


class AttribSysStichHint(AttribSys):
    def __init__(self):
        super(AttribSysStichHint, self).__init__()


class AttribSysTag(AttribSys):
    def __init__(self):
        super(AttribSysTag, self).__init__()


class AttribSysVertedge(AttribSys):
    def __init__(self):
        super(AttribSysVertedge, self).__init__()


class AttribTsl(Attrib):
    def __init__(self):
        super(AttribTsl, self).__init__()
...
class AttribNamingMatchingNMxMatchedEntity(AttribNamingMatching):
    # [dxIdx, msk] n1, n2
    def __init__(self):
        super(AttribNamingMatchingNMxMatchedEntity, self).__init__()
...
class AttribNamingMatchingNMxEdgeCurve(AttribNamingMatching):
    def __init__(self):
        super(AttribNamingMatchingNMxEdgeCurve, self).__init__()


class AttribNamingMatchingNMxDup(AttribNamingMatching):
    # n1, n2, n3, n4, n5
    def __init__(self):
        super(AttribNamingMatchingNMxDup, self).__init__()


class AttribNamingMatchingNMxOrderCount(AttribNamingMatching):
    # n1, n2
    def __init__(self):
        super(AttribNamingMatchingNMxOrderCount, self).__init__()


class AttribNamingMatchingNMxPuchtoolBRep(AttribNamingMatching):
    # n1, n2
    def __init__(self):
        super(AttribNamingMatchingNMxPuchtoolBRep, self).__init__()
...
class AttribNamingMatchingNMxThreadEntity(AttribNamingMatching):
    # x1, x2, V1, V2, n1, V2, [????]
    def __init__(self):
        super(AttribNamingMatchingNMxThreadEntity, self).__init__()
...
class AttribNamingMatchingNMxTagWeldLateralFaceName(AttribNamingMatching):
    # no more values
    def __init__(self):
        super(AttribNamingMatchingNMxTagWeldLateralFaceName, self).__init__()


class AttribNamingMatchingNMxTagWeldLumpFaceName(AttribNamingMatching):
    # no more values
    def __init__(self):
        super(AttribNamingMatchingNMxTagWeldLumpFaceName, self).__init__()


class AttribNamingMatchingNMxWeld(AttribNamingMatching):
    # n
    def __init__(self):
        super(AttribNamingMatchingNMxWeld, self).__init__()


class AttribNamingMatchingNMxWireTag(AttribNamingMatching):
    # float
    def __init__(self):
        super(AttribNamingMatchingNMxWireTag, self).__init__()
...
class AttribNamingMatchingNMxGenTagDisambiguation(AttribNamingMatching):
    # n1
    def __init__(self):
        super(AttribNamingMatchingNMxGenTagDisambiguation, self).__init__()


class AttribNamingMatchingNMxFeatureDependency(AttribNamingMatching):
    # [a]
    def __init__(self):
        super(AttribNamingMatchingNMxFeatureDependency, self).__init__()
...
class AnnotationPrimitive(Annotation):
    def __init__(self):
        super(AnnotationPrimitive, self).__init__()


class AnnotationSplit(Annotation):
    def __init__(self):
        super(AnnotationSplit, self).__init__()


class AnnotationTol(Annotation):
    def __init__(self):
        super(AnnotationTol, self).__init__()


class AnnotationTolCreate(AnnotationTol):
    def __init__(self):
        super(AnnotationTolCreate, self).__init__()


class AnnotationTolRevert(AnnotationTol):
    def __init__(self):
        super(AnnotationTolRevert, self).__init__()


class AttribCt(Attrib):
    def __init__(self):
        super(AttribCt, self).__init__()


class AttribCtCellPtr(AttribCt):
    def __init__(self):
        super(AttribCtCellPtr, self).__init__()


class AttribCtCFace(AttribCt):
    def __init__(self):
        super(AttribCtCFace, self).__init__()
...
class AcisChunkUtf8U8(_AcisChunk_):
    ...
    def __str__(self):
        return "@%d %s " % (len(self.val), self.val)
...
class AcisChunkSubtypeOpen(_AcisChunk_):
    ...
    def __init__(self, value=None):
        super(AcisChunkSubtypeOpen, self).__init__(TAG_SUBTYPE_OPEN, "{")
...
class AcisChunkSubtypeClose(_AcisChunk_):
    ...
    def __init__(self, value=None):
        super(AcisChunkSubtypeClose, self).__init__(TAG_SUBTYPE_CLOSE, "}")
...
class AcisChunkTerminator(_AcisChunk_):
    ...
    def __init__(self, value=None):
        super(AcisChunkTerminator, self).__init__(TAG_TERMINATOR, "#")
...
class _AcisChunkArray_(_AcisChunk_):
    ...
    def __repr__(self):
        return "%s " % (" ".join(["%g" % (f) for f in self.val]))
...
class AcisChunkPosition(_AcisChunkArray_):
    ...
    def __str__(self):
        s = getScale()
        return "%s " % (" ".join(["%g" % (f * s) for f in self.val]))
...
class History(object):
    ...
    def __str__(self):
        return "SAT %s: %d %d %d %d %s %s %s %s" % (
            self.history_stream,
            self.current_state,
            self.next_state,
            self.keep_max_states,
            self.unknown,
            self.ds_current,
            self.ds_root,
            self.ds_active,
...
class AcisReader(object):
    ...
    def _translateChunk(self, token):
        ...
        if token == "(":
            ...
            if tokZ == ")":
                return TAG_VECTOR_2D, [float(tokX), float(tokY)]
    ...
    def _readHeaderText(self):
        ...
        if self.version >= 2.0:
            ...
            data = self._stream.readline()
            tokens = data.split(" ")
            self.header.scale = fabs(
                float(tokens[0])
            )  # prevent STEP importer from handling negative scales -> "Cannot compute Inventor representation for the shape of Part__Feature"
...
VBL_CLASSES = {"circle": BDY_GEOM_CIRCLE, "deg": BDY_GEOM_DEG, "pcurve": BDY_GEOM_PCURVE, "plane": BDY_GEOM_PLANE}

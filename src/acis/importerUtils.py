...
import os, sys, datetime, json, shutil, re
from struct            import Struct, unpack_from, pack
from .geometry         import Vector as VEC
...
def _log(caller, method, msg, args):
    try:
        ...
        print("FATAL ERROR in %s:\n" %(caller))
        print("msg   = " + msg)
        if (len(args) > 0): print("*args = (%s)" %(",".join(args)))
...
def logInfo(msg, *args):
    _log("logInfo",    print, msg, args)
def logWarning(msg, *args):
    _log("logWarning", print, msg, args)
def logError(msg, *args):
    _log("logError",   print,   msg, args)

def logAlways(msg, *args):
    _log("logAlways", print, msg, args)
